...
class UserWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def add(self):
        ...
        if userName != '' and userPwd != '':
            result = service.exec_db('insert into tb_user (userName, userPwd) values (%s, %s)',
                                     (userName, userPwd))
            if result > 0:
                self.query_db()
    ...
    def edit(self):
        try:
            if self.select != '':
                userPwd = self.lineEdit_pwd.text()
                if userPwd != '':
                    result = service.exec_db('update tb_user set userPwd=%s '
                                             'where userName=%s', (userPwd, self.select, ))
                    if result > 0:
                        self.query_db()
    ...
    def delete(self):
        try:
            if self.select != '':
                result = service.exec_db('delete from tb_user where userName=%s', (self.select, ))
                if result > 0:
                    self.query_db()
