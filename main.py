...
class MainWindow(QMainWindow, Ui_mainWindow):
    def __init__(self):
        ...
        self.btn_print_preview.clicked.connect(self.print_left)
        self.btn_print.clicked.connect(self.print_right)
    ...
    # def SUBSTRING(self, input_string, start, length):  # 截断函数，功能相当于mysql的SUBSTRING()函数。
    ...
    def get_item(self, item):
        if item.column() == 0:
            ...
            result = service.query_db("select * from tb_device where dev_id=%s", item.text())
    ...
    def export_excel(self):
        ...
        result1, result2 = service.query_desc(sql)
    ...
    def show_all(self):
        ...
        if name == "所有":
            ...
            if location == "所有":
                result = service.query_db("select dev_id, dev_name, location, control_range, phone from tb_device "
                                          "where dev_name=%s", name)
            else:
                result = service.query_db("select dev_id, dev_name, location, control_range, phone from tb_device "
                                          "where dev_name=%s and location=%s", name, location)
        row = len(result)
    ...
    def add(self):
        ...
        if dev_name != "" and location != "" and control_range != "" and phone != "":
            sql = "select count(*) from tb_device where dev_name=%s and location=%s and control_range=%s"
            ...
            if count >= 1:
                ...
                if result > 0:
                    sql = "select dev_id from tb_device where dev_name=%s and location=%s and control_range=%s " \
                          "and phone=%s"
